...
fn run_signal_loop() -> io::Result<fd::OwnedFd> {
...
fn reap_zombies() {
    loop {
        match rx_process::waitid(
            rx_process::WaitId::All,
            rx_process::WaitIdOptions::EXITED | rx_process::WaitIdOptions::NOHANG,
        ) {
            Ok(Some(status)) => log_reaped(&status),
            Ok(None) => break,
